...
- 库: 已完成(全部 17 个单元测试通过, Noise 规范 §7 的 12 种模式均已
  暴露)。
- 配置: 已完成(`base.proto`、`clientcfg.proto`、`servercfg.proto`)。
- 集成到 `pkg/protocol/underlay_stream.go` 和
  `pkg/protocol/underlay_packet.go`: 设计是在 `Session` 上新增工厂,
  根据 `ClientProfile.encryption` / `ServerConfig.encryption` 决定
  返回一对 `BlockCipher`。此部分尚未完成, 当前代码仍使用密码派生的
  cipher。跟踪 issue: TBD。
