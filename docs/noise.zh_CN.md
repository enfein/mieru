...
- 协议集成: **TCP 已完成**。 mux 在每次新建立的连接上(客户端 Dial
  或服务器 Accept)先执行 Noise 握手, 之后 mieru 报文才开始流动。
  握手得到的传输 `CipherState` 对被适配成两个 `cipher.BlockCipher`
  并直接装入流 underlay 的 send/recv 槽位, 绕过密码派生。参见
  [`pkg/protocol/noise_wire.go`](../pkg/protocol/noise_wire.go) 以及
  [`pkg/protocol/mux.go`](../pkg/protocol/mux.go) 中的 `usesNoise()`
  分支。
- UDP: 暂不支持。连接时显式报错, 信息为 `"noise encryption is not
  supported over UDP yet; use a TCP port binding or switch encryption
  to XCHACHA20_POLY1305"`。带 per-source 状态跟踪的数据报握手属于
  后续增强。
- 端到端: [`test/deploy/noise_e2e/`](../test/deploy/noise_e2e) 构建
  真实的 `mieru` + `mita` 二进制到容器中, 使用 Noise_XX JSON 配置,
  通过 Noise 隧道运行 SOCKS5 流量(`make run-container-test-noise-e2e`)。
