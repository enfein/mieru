...
// timedFuncHeap is a heap for sorted timed function.
...
type TimedSched struct {
	prependTasks    []timedFunc // prepending tasks
...
// sched executes the tasks when the time has come.
	...
	for {
		...
		case task := <-ts.chTask:
			...
			if now.After(task.ts) {
				// The task is already delayed! Execute immediately.
				task.execute()
			} else {
...
// prepend adds new tasks into appropriate positions in the heap.
