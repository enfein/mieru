...
const (
	...
	ServerNotRunningWithCommand             = "mieru server daemon is not running; please run command \"sudo systemctl restart mita\" to start server daemon; run command \"sudo journalctl -e -u mita --no-pager\" to check log if unable to start"
